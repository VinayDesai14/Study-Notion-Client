...
function Navbar() {
  ...
  useEffect(() => {
     ...
     const getAllCategories=async () => {
      setLoading(true)
      try {
        const res = await apiConnector("GET", categories.CATEGORIES_API, null, null, null, false)
              ...
              <li key={index}>
                {link.title === "Catalog" ? (
                  <>
                    ...
                    >
                       <p>{link.title}</p>
                      <BsChevronDown className="arrow  transition-transform duration-300 ease-in-out group-hover:rotate-180" />
